...
def play_wave(filename="sound.wav", player="paplay"):
    """Plays a wave file.
...
class Wave:
    ...
    def play(self, /, save=False, filename="sound.wav"):
        """Plays a wave file.
        ...
        if save:
            ...
            INT16_MAX = 32767
            audio = (np.clip(self.ys, -1, 1) * INT16_MAX).astype(np.int16)
            p = subprocess.Popen(
                [
                    "paplay",
                    "--raw",
...
class Signal(ABC):
    ...
    def make_wave(cls, duration=1, start=0, framerate=11025):
        """Makes a Wave object.
